...
public interface ITenantFileVersionPolicyForDocumentLibraryService
{
    ...
    TenantOperationResult SetObject(Uri siteUrl, string listTitle, IReadOnlyDictionary<string, object> modificationInfo);
...
public class TenantFileVersionPolicyForDocumentLibraryService(ClientContext clientContext) : TenantClientService(clientContext), ITenantFileVersionPolicyForDocumentLibraryService
{
    ...
    public TenantOperationResult SetObject(Uri siteUrl, Guid listId, IReadOnlyDictionary<string, object> modificationInfo)
    {
        _ = siteUrl ?? throw new ArgumentNullException(nameof(siteUrl));
        var requestPayload = new ClientRequestPayload();
        var objectPath1 = requestPayload.Add(
        ...
        var objectPath2 = requestPayload.Add(
            new ObjectPathMethod(
                ...
                requestPayload.CreateParameter(siteUrl),
                requestPayload.CreateParameter(new ListParameters(listId)),
                requestPayload.CreateParameter(new FileVersionPolicyForDocumentLibrary(modificationInfo))),
    ...
    public TenantOperationResult SetObject(Uri siteUrl, string listTitle, IReadOnlyDictionary<string, object> modificationInfo)
    {
        _ = siteUrl ?? throw new ArgumentNullException(nameof(siteUrl));
        _ = listTitle ?? throw new ArgumentNullException(nameof(listTitle));
        var requestPayload = new ClientRequestPayload();
