...
namespace Karamem0.SharePoint.PowerShell.Commands
{
    ...
    public class GetFolderCommand : ClientObjectCmdlet<IFolderService>
    {
        ...
        protected override void ProcessRecordCore(ref List<object> outputs)
        {
            ...
            {
                outputs.Add(this.Service.GetObject(this.List));
            }
            if (this.ParameterSetName == "ParamSet3")
            {
                outputs.Add(this.Service.GetObject(this.FolderId));
            }
            if (this.ParameterSetName == "ParamSet4")
            {
                if (this.FolderUrl.IsAbsoluteUri)
                {
                    outputs.Add(this.Service.GetObject(new Uri(this.FolderUrl.AbsolutePath, UriKind.Relative)));
                }
                else
                {
                    outputs.Add(this.Service.GetObject(this.FolderUrl));
                }
            }
            if (this.ParameterSetName == "ParamSet5")
            {
                outputs.Add(this.Service.GetObject(this.Folder, this.FolderName));
            }
