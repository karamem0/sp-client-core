...
namespace Karamem0.SharePoint.PowerShell.Commands
{
    ...
    public class GetNavigationNodeCommand : ClientObjectCmdlet<INavigationNodeService>
    {
        ...
        protected override void ProcessRecordCore(ref List<object> outputs)
        {
            ...
            {
                outputs.Add(this.Service.GetObject(this.NavigationNodeId));
            }
            if (this.ParameterSetName == "ParamSet3")
            {
                if (this.NoEnumerate)
                {
                    outputs.Add(this.Service.GetObjectEnumerable(this.NavigationNode));
                }
                else
                {
                    outputs.AddRange(this.Service.GetObjectEnumerable(this.NavigationNode));
                }
            }
        }
