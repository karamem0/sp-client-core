...
Adds an allowed content type for a document set.
...
```powershell
PS C:\> Add-KshDocumentSetAllowedContentType -ContentType $contentType -AllowedContentType $allowedContentType -PushChanges
```

This example adds the specified content type as an allowed content type to the specified content type and pushes the changes.
